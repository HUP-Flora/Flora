...
import {
  giftCardState, paymentAmountState, receiveUserAddressState,
...
import {
  ...
  MarginBottom8TextInput, SubmitPaymentButton, TextInput
...
function SecondDeliveryForm({time}) {
  ...
  const [receiveUserAddress, setReceiveUserAddress] = useRecoilState(
  ...
  const ThirdDeliveryFormHandler = (e) => {
    const formData = [
      ...
      { key: 'receiveUserPhone', value: receiveUserPhone, toggleError: VreceiveUserPhoneToggleHasError },
    ...
    <>
      <FormWrapper>
        ...
        <FormContent>
          ...
          />
          <InputCounterContainer>
            {VreceiveUserHasError && (
              <ErrorMessage>받는 분을 입력해주세요.</ErrorMessage>
            )}
          ...
          <TextInput type="text" id="paymentAmount" placeholder="내용을 입력해주세요." onChange={e => {
          ...
          HasError={VpaymentAmountHasError}
          />
          {VpaymentAmountHasError && (
            <ErrorMessage>결제 금액을 입력해주세요.</ErrorMessage>
          )}
          <SubmitPaymentButton onClick={e => {
            ThirdDeliveryFormHandler(e);
          }}>결제하기</SubmitPaymentButton>
        </FormContent>
      </FormWrapper>
      <FormTime>
        {time}
